...
preloaderInit();
  ...
  $('.gallery-slider').slick({
      ...
      speed: 4000,          // controls scrolling speed
      cssEase: 'linear',    // smooth nonstop movement
      infinite: true,
      arrows: false,
      dots: false,
      pauseOnHover: false,
      pauseOnFocus: false,
      swipe: false,         // optional: prevents breaking smooth flow
      responsive: [
          {
              breakpoint: 1100,
              settings: {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    filters.forEach(filter => {
        filter.addEventListener('click', function () {
            ...
            items.forEach(item => {
                item.style.display =
                    (value === 'all' || item.getAttribute('data-name') === value)
                    ? 'inline-block'
                    : 'none';
            });
        });
